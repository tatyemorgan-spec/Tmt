# Close SMS templates: which one when (Antoinette's set, in Tye's name)

Fill every [DAY] / [TIME] / [LINK] before sending.

## VSL leads
| Situation | Template |
|---|---|
| New app, out of hours | Tye — Speed to Lead (New Application) |
| Missed call 1 | Tye — VSL Follow Up 1 |
| Missed call 2 | Tye — VSL Follow Up 2 ("right person?") |
| Missed call 3 | Tye — VSL Follow Up 3 (last try) → then NGMI |
| Booked themselves, need to chat first | Tye — Booked Call: Quick Chat First |
| Booked, still can't reach | Tye — Booked Call: Still Trying |
| Booked, ignoring calls + texts | Tye — Booked Call: Not Answering (Slot Warning) |
| Still nothing → cancel | Tye — Cancelling Call (Couldn't Reach) |
| Spoke, sending videos (£7k+) | Tye — Videos: Airbnb Only |
| Spoke, sending videos (lower funds) | Tye — Videos: Airbnb + Management |
| Missed call after videos | Tye — Check In After Videos |
| Booked + watched videos? chase | Tye — Booked Call: Chase After Videos |

## Booked with Freeman
| When | Template |
|---|---|
| Straight after you book them | Tye — Booked: Call Confirmation |
| Day before | Tye — Reminder: Call Tomorrow |
| Morning of | Tye — Reminder: Call Today |
| A few minutes before | Tye — Reminder: Few Minutes |
| No-show | Tye — No Show Rebook |

## Webinar leads
| When | Template |
|---|---|
| They booked your discovery call | Tye — Webinar: Discovery Call Booked |
| Morning of your discovery call | Tye — Webinar: Discovery Call Today |
| Never reached | Tye — Webinar: No Contact (YouTube) |

## After the sale
| When | Template |
|---|---|
| Instalment due, no answer | Tye — Instalment Check-in |

## Still useful from before
Our Reviews · Student Review or Case Study · Management Pitch · Tye — Spoke: Thinking About It · Tye — Spoke: Busy With Work · Tye — Spoke: Callback Confirmed · Tye — Not Interested (Door Open) · Tye — Nurture Check-in (2 weeks) · Pipeline Leads

## Proof texts (new)
| When | Template |
|---|---|
| Worried about time / shift worker | Tye — Proof: Amara (Shift Workers / Time) |
| "Is this legit?" | Tye — Proof: Is It Legit |
